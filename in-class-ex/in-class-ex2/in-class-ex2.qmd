...
The 6 × 4 grid does not produce 24 quadrats. The observation window is the Singapore coastline, not a rectangle, so cells falling entirely in the sea are dropped and the ones along the coast are clipped to irregular shapes. Several of the surviving tiles hold no centres at all.
...
The direction of the answer is stable: every grid rejects CSR, and every VMR is far above 1, so the conclusion that childcare centres are clustered does not depend on the choice. The magnitude is not stable at all, so the grid that produced a VMR is part of the result rather than a detail behind it.

The peak at 4 × 4 comes from the variance column, which reaches 31371 there — higher than at any other grid, including the coarser 2 × 2. The 2 × 2 grid has only four tiles and every one of them mixes dense and empty land, so no tile is empty and the counts stay comparatively close together. The 4 × 4 grid is the first that is fine enough to isolate tiles containing nothing at all, while still leaving tiles holding over 500 centres; that combination maximises the spread. Beyond it, finer grids cut the largest counts down faster than they cut the mean, and the ratio falls away.
:::
